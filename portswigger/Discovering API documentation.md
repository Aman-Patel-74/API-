...
## 5. Discovering API Documentation

Even when API documentation isn't openly published, it may still be
accessible by browsing the applications that consume the API.

### Techniques
- **Burp Scanner** — crawl the API to automatically discover content,
  including hidden or linked documentation.
- **Manual browsing** — use Burp's built-in browser to explore the
  target application by hand, watching for links or references to
  documentation.
...
### Investigating the Base Path
If you find an endpoint for a specific resource, walk back through its
base path to look for documentation or overview endpoints at each
level.

Example — given the resource endpoint:
```
...
### Using Intruder for Discovery
- Use a **list of common documentation paths** as payloads in Burp
  Intruder to brute-force/discover documentation endpoints that
  aren't linked anywhere in the app.

## 6. Common Sources for Recon Info
