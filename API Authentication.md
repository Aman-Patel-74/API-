# API Authentication

## Overview

**API authentication** is the process of verifying the identity of the user or application making an API request.

- **Authentication** answers the question: *Who are you?*
- **Authorization** answers the question: *What are you allowed to do?*

## Common API Authentication Methods

1. [Basic Authentication](#1-basic-authentication)
2. [API Keys](#2-api-keys)
3. [OAuth 2.0](#3-oauth-20)
4. [JWT (JSON Web Tokens)](#4-jwt-json-web-tokens)
5. [Certificate-Based Authentication (Mutual TLS / mTLS)](#5-certificate-based-authentication-mutual-tls--mtls)
6. [HMAC Authentication](#6-hmac-authentication)
7. [Session-Based Authentication](#7-session-based-authentication)

---

## 1. Basic Authentication

### How It Works

The client sends the username and password encoded in Base64 inside the `Authorization` header.

**Example:**
```
Authorization: Basic dXNlcjpwYXNzd29yZA==
```
*(Base64 of `user:password`)*

### Pros
- Very simple to implement
- Suitable for internal or low-security APIs
- Useful for testing environments

### Cons
- Credentials are sent with every request
- Not secure without HTTPS
- Vulnerable to credential leakage if intercepted

### Security Notes
- Must always be used over HTTPS
- Avoid for public production APIs
- Susceptible to brute force if no rate limiting

---

## 2. API Keys

### How It Works
- Each client receives a unique API key.
- The key is sent in:
  - Header
  - Query parameter
  - Request body (less common)

**Example:**
```
GET /api/data
x-api-key: abc123xyz
```

### Pros
- Simple implementation
- Good for service-to-service communication
- Useful for read-only APIs

### Cons
- Static credential
- No built-in user identity
- Limited fine-grained authorization
- Often exposed in client-side JavaScript

### Security Risks
- API key leakage in source code
- Exposure in browser DevTools
- Logging exposure
- No expiration by default

---

## 3. OAuth 2.0

OAuth 2.0 is an **authorization framework**, not strictly an authentication protocol. It allows third-party applications to access resources without exposing user credentials.

### How It Works
1. User authenticates with authorization server.
2. Server issues an access token.
3. Client uses the access token in API requests.

**Example:**
```
Authorization: Bearer access_token_here
```

### Common Grant Types
- Authorization Code (most secure for web apps)
- Client Credentials (machine-to-machine)
- Device Code
- Refresh Token
- *(Implicit and Password grants are deprecated in modern implementations)*

### Pros
- Granular access control via scopes
- Token expiration and refresh support
- Widely adopted standard

### Cons
- Complex implementation
- Misconfiguration risks
- Token leakage risk

### Security Risks
- Missing state validation
- Token reuse
- Open redirect vulnerabilities
- Improper scope validation

---

## 4. JWT (JSON Web Tokens)

JWT is a compact, URL-safe token format used for authentication and authorization. JWTs are **self-contained tokens**.

### Structure of a JWT

A JWT consists of three parts:
```
Header.Payload.Signature
```

#### 1. Header
Specifies:
- Token type
- Signing algorithm

**Example:**
```json
{
  "alg": "HS256",
  "typ": "JWT"
}
```

#### 2. Payload
Contains claims:
- User ID
- Roles
- Expiration time
- Issuer

**Example:**
```json
{
  "sub": "101",
  "role": "admin",
  "exp": 1700000000
}
```

#### 3. Signature
Generated by signing the header and payload using a secret or private key.

### Pros
- Stateless
- No server-side session storage
- Fast validation
- Suitable for microservices

### Cons
- Cannot be revoked easily unless tracked
- Large token size if many claims included

---

## 5. Certificate-Based Authentication (Mutual TLS / mTLS)

### How It Works
Both client and server present digital certificates. The server verifies the client certificate before allowing access.

### Pros
- Very strong authentication
- Cryptographic identity verification
- Ideal for sensitive environments

### Cons
- Complex certificate lifecycle management
- Revocation handling required
- Operational overhead

### Common Use Cases
- Banking systems
- Enterprise integrations
- Microservice internal communication

---

## 6. HMAC Authentication

HMAC uses a shared secret to sign requests.

### How It Works
1. Client creates a hash using:
   - Secret key
   - Request components (method, path, timestamp, body)
2. Server recalculates and verifies the hash.

**Example header:**
```
Authorization: HMAC signature_here
```

### Pros
- Ensures integrity
- Prevents tampering
- Protects against replay attacks (with timestamp)

### Cons
- Shared secret management required
- Computational overhead

### Common Use Cases
- Cloud APIs
- Financial APIs
- AWS-style request signing

---

## 7. Session-Based Authentication

### How It Works
1. User logs in.
2. Server creates session ID.
3. Session ID stored server-side.
4. Client stores session ID in a cookie.
5. Cookie sent with each request.

### Pros
- Easy to revoke sessions
- Server controls session lifecycle
- Common in traditional web apps

### Cons
- Not scalable without distributed session store
- Not ideal for pure REST APIs
- Requires server-side state

### Cookie-Based Authentication

Cookie-based authentication relies on storing a session identifier in a browser cookie.

**Flow:**
1. User logs in.
2. Server creates session ID.
3. Session ID sent in `Set-Cookie`.
4. Browser stores cookie.
5. Cookie sent automatically with each request.

**Example:**
```
Set-Cookie: sessionId=abc123; HttpOnly; Secure
```

#### Best Practices
- Use HTTPS only
- Set `Secure` flag
- Set `HttpOnly` flag
- Use `SameSite` attribute
- Use strong random session IDs
- Set expiration time
- Invalidate session on logout
- Rotate session ID after login

#### Common Security Risks
- Session hijacking
- Cross-Site Scripting (XSS)
- Cross-Site Request Forgery (CSRF)
- Session fixation
- Predictable session IDs

### Token-Based Authentication

Token-based authentication uses a token instead of a server-side session.

**Flow:**
1. User logs in.
2. Server generates token.
3. Client stores token (local storage or cookie).
4. Client sends token in header.

**Example:**
```
Authorization: Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9...
```

#### Advantages
- Stateless
- Scalable
- Suitable for APIs and SPAs
- Works well with microservices

#### Security Best Practices
- Always use HTTPS
- Set expiration time
- Validate signature
- Validate issuer and audience
- Implement token rotation
- Invalidate on logout
- Store securely (avoid `localStorage` if possible)

#### Common Risks
- Token theft via XSS
- Long-lived tokens
- Missing expiration validation
- Accepting unsigned tokens
- Storing sensitive claims

---

## Cookie-Based vs Token-Based Authentication

| Feature       | Cookie-Based       | Token-Based                  |
|---------------|---------------------|-------------------------------|
| Server State  | Stateful            | Stateless                     |
| Storage       | Browser cookie      | Header or storage              |
| Scalability   | Limited             | High                           |
| Revocation    | Easy                | Harder                         |
| CSRF Risk     | Higher              | Lower (if header-based)        |
| XSS Risk      | Lower (HttpOnly)    | Higher (if localStorage)       |

---

## Choosing the Right Authentication Method

The correct choice depends on:
- Application type
- Security requirements
- Scalability needs
- Architecture (monolith vs microservices)
- Third-party integration requirements

### General Guidance

| Scenario                     | Recommended Method       |
|-------------------------------|---------------------------|
| Internal APIs                 | mTLS or HMAC              |
| Public APIs                   | OAuth 2.0 + JWT            |
| Traditional web apps          | Session + Cookies          |
| Microservices                 | JWT or mTLS                |
| High-security environments    | mTLS + OAuth               |
